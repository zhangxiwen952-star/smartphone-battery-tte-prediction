...
def main():
    ...
    temp = tc.evaluate_all_scenarios(
        scenario_df, sample_df, power, E_eff,
        temp_model=temp_model, soh_model=None,
        residual_power_w=0.0, options=options, max_scenarios=args.max_scenarios
    ).rename(columns={"t_empty_hours": "t_empty_temp"})

    merged = pd.merge(
        base[["scenario_id", "temp_c", "t_empty_base", "mae_soc", "rmse_soc"]],
        temp[["scenario_id", "t_empty_temp"]],
        on="scenario_id", how="inner"
    )
    merged["delta_hours"] = merged["t_empty_temp"] - merged["t_empty_base"]
    merged["delta_percent"] = np.where(
        merged["t_empty_base"] > 0, merged["delta_hours"] / merged["t_empty_base"] * 100.0, np.nan
    )

    tc.save_csv(merged, os.path.join(outdir, "exp01_temperature_per_scenario.csv"))
    tc.save_json({
        "power_model_metrics": metrics,
        "E_eff_wh": E_eff,
        "baseline_summary": tc.summarize(base.rename(columns={"t_empty_base": "t_empty_hours"})),
        "temp_summary": tc.summarize(temp.rename(columns={"t_empty_temp": "t_empty_hours"})),
        "delta_mean_percent": float(np.nanmean(merged["delta_percent"])),
        "delta_median_percent": float(np.nanmedian(merged["delta_percent"])),
    }, os.path.join(outdir, "exp01_temperature_summary.json"))

    print("Saved exp01 results to:", os.path.abspath(outdir))
